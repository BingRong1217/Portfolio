...
GAME_START_STR DB '  ',0ah,0dh
  ...
  db   '                                                                    ',0ah,0dh
  DB   '                ====================================================',0ah,0dh
  DB   '               ||                   MUMMY SAYS                     ||',0ah,0dh                                        
  DB   '               ||            I CAN NOT HAVE BOYFRIENDS             ||',0ah,0dh
  DB   '               ||                UNTIL COLLEGE !!!                 ||',0ah,0dh
  DB   '               ||--------------------------------------------------||',0ah,0dh
  DB   '               ||                                                  ||',0ah,0dh
  DB   '               ||        Please Enter the name of the girl         ||',0ah,0dh
  DB   '               ||                then press Enter                  ||',0ah,0dh   
  DB   '               ||       Please Enter the name of the mummy         ||',0ah,0dh
  DB   '               ||                                                  ||',0ah,0dh
  DB   '               ||                                                  ||',0ah,0dh
...
INCLUDE classr~1.INC

INCLUDE lab.INC

INCLUDE playgr~1.INC
...
PLAYER1 db 'A          Girl NAME:','$'
player2 db 'B         Mummy NAME:','$'
...
powerup2  DB BGC,BGC,BGC,BGC,BGC,BGC,BGC,BGC,BGC,BGC,BGC,BGC
	      DB BGC,BGC,BGC,BLACK,BLACK,BLACK,BLACK,BLACK,BLACK,BGC,BGC,BGC
	      DB BGC,BGC,BGC,BGC,BLACK,Dark_Gray,Dark_Gray,BLACK,BGC,BGC,BGC,BGC
	      DB BGC,BGC,BGC,BGC,BLACK,Dark_Gray,Dark_Gray,BLACK,BGC,BGC,BGC,BGC
	      DB BGC,BGC,BGC,BGC,BLACK,Dark_Gray,Dark_Gray,BLACK,BGC,BGC,BGC,BGC
	      DB BGC,BGC,BGC,BLACK,Dark_Gray,RED,RED,Dark_Gray,BLACK,BGC,BGC,BGC
	      DB BGC,BGC,BLACK,Dark_Gray,RED,RED,RED,RED,Dark_Gray,BLACK,BGC,BGC
	      DB BGC,BGC,BLACK,Dark_Gray,RED,WHITE,RED,RED,Dark_Gray,BLACK,BGC,BGC
	      DB BGC,BGC,BLACK,Dark_Gray,Dark_Gray,RED,RED,Dark_Gray,Dark_Gray,BLACK,BGC,BGC
	      DB BGC,BGC,BGC,BLACK,BLACK,Dark_Gray,Dark_Gray,BLACK,BLACK,BGC,BGC,BGC
    ...
    GAME_BEGIN:      ;GAME MENU SCREEN   
        ...
        INFLOOP:
            ...
            MOVINGLOOP:
				...
				lab_scene:
					cmp SCENE, 1
					jne playground_scene				
					call DRAW_MAP_lab
					jmp scene_OK
					
				playground_scene:
					call DRAW_MAP_playground
...
DRAW_MAP_class_unit_1 proc near

	push dx
	
	mov si, 0
	
	DRAW_MAP_class_unit_1_Y:
		mov di, 0
		
		DRAW_MAP_class_unit_1_X:
			MOV AL, classroom1shape[si]
		...
		add di, 20
		cmp dx, di
	jnz DRAW_MAP_class_unit_1_Y
	
	pop dx
	ret
DRAW_MAP_class_unit_1 endp

DRAW_MAP_class_unit_2 proc near

	push dx
	
	mov si, 0
	
	DRAW_MAP_class_unit_2_Y:
		mov di, 0
		
		DRAW_MAP_class_unit_2_X:
			MOV AL, classroom2shape[si]
		...
		add di, 20
		cmp dx, di
	jnz DRAW_MAP_class_unit_2_Y
	
	pop dx
	ret
DRAW_MAP_class_unit_2 endp

DRAW_MAP_class proc near  
    ...
    DRAW_MAP_class_STEP1:
		MOV DX, STEPBEGINY
		call DRAW_MAP_class_unit_1
	
        add CX, 20
        CMP CX, 140D
    JNZ DRAW_MAP_class_STEP1

    MOV CX, 180D
    DRAW_MAP_class_STEP2: 
		MOV DX, STEPBEGINY
		call DRAW_MAP_class_unit_1

        add CX, 20
        CMP CX, 280D
    JNZ DRAW_MAP_class_STEP2  


    MOV CX, 90D
    DRAW_MAP_class_STEP3: 
		mov DX, (130)
		call DRAW_MAP_class_unit_2	
        add CX, 20
        CMP CX, 230D
    JNZ DRAW_MAP_class_STEP3



    MOV CX, 40D
    DRAW_MAP_class_STEP4: 
		mov DX, (170)
		call DRAW_MAP_class_unit_1	
        add CX, 20
        CMP CX, 140D
    JNZ DRAW_MAP_class_STEP4

    
    MOV CX, 180D
    DRAW_MAP_class_STEP5: 
		mov DX, (170)
		call DRAW_MAP_class_unit_1	
        add CX, 20
        CMP CX, 280D
    JNZ DRAW_MAP_class_STEP5  
...
DRAW_MAP_lab_unit_1 proc near

	push dx
	
	mov si, 0
	
	DRAW_MAP_lab_unit_1_Y:
		mov di, 0
		
		DRAW_MAP_lab_unit_1_X:
			MOV AL, lab1shape[si]
		...
		add di, 20
		cmp dx, di
	jnz DRAW_MAP_lab_unit_1_Y
	
	pop dx
	ret
DRAW_MAP_lab_unit_1 endp

DRAW_MAP_lab_unit_2 proc near

	push dx
	
	mov si, 0
	
	DRAW_MAP_lab_unit_2_Y:
		mov di, 0
		
		DRAW_MAP_lab_unit_2_X:
			MOV AL, lab2shape[si]
		...
		add di, 20
		cmp dx, di
	jnz DRAW_MAP_lab_unit_2_Y
	
	pop dx
	ret
DRAW_MAP_lab_unit_2 endp

DRAW_MAP_lab proc near  
    ...
    DRAW_MAP_lab_STEP1:
		MOV DX, STEPBEGINY
		call DRAW_MAP_lab_unit_1
	
        add CX, 20
        CMP CX, 140D
    JNZ DRAW_MAP_lab_STEP1
    ...
    DRAW_MAP_lab_STEP2: 
        ...
        add CX, 20
        CMP CX, 280D
    JNZ DRAW_MAP_lab_STEP2  


    MOV CX, 90D
    DRAW_MAP_lab_STEP3: 
		mov DX, (130)
		call DRAW_MAP_lab_unit_2	
        add CX, 20
        CMP CX, 230D
    JNZ DRAW_MAP_lab_STEP3
...
DRAW_MAP_playground_unit_1 proc near

	push dx
	
	mov si, 0
	
	DRAW_MAP_playground_unit_1_Y:
		mov di, 0
		
		DRAW_MAP_playground_unit_1_X:
			MOV AL, playground1shape[si]
			...
			inc di
			cmp di, 20
		jnz DRAW_MAP_playground_unit_1_X
		...
		add di, 20
		cmp dx, di
	jnz DRAW_MAP_playground_unit_1_Y
	
	pop dx
	ret
DRAW_MAP_playground_unit_1 endp

DRAW_MAP_playground_unit_2 proc near

	push dx
	
	mov si, 0
	
	DRAW_MAP_playground_unit_2_Y:
		mov di, 0
		
		DRAW_MAP_playground_unit_2_X:
			MOV AL, playground2shape[si]
			...
			inc di
			cmp di, 20
		jnz DRAW_MAP_playground_unit_2_X
		...
		add di, 20
		cmp dx, di
	jnz DRAW_MAP_playground_unit_2_Y
	
	pop dx
	ret
DRAW_MAP_playground_unit_2 endp

DRAW_MAP_playground proc near  
    ...
    DRAW_MAP_playground_STEP1:
		MOV DX, STEPBEGINY
		call DRAW_MAP_playground_unit_1
	
        add CX, 20
        CMP CX, 140D
    JNZ DRAW_MAP_playground_STEP1

    MOV CX, 180D
    DRAW_MAP_playground_STEP2: 
		MOV DX, STEPBEGINY
		call DRAW_MAP_playground_unit_1

        add CX, 20
        CMP CX, 280D
    JNZ DRAW_MAP_playground_STEP2  


    MOV CX, 90D
    DRAW_MAP_playground_STEP3: 
		mov DX, (130)
		call DRAW_MAP_playground_unit_2	
        add CX, 20
        CMP CX, 230D
    JNZ DRAW_MAP_playground_STEP3



    MOV CX, 40D
    DRAW_MAP_playground_STEP4: 
		mov DX, (170)
		call DRAW_MAP_playground_unit_1	
        add CX, 20
        CMP CX, 140D
    JNZ DRAW_MAP_playground_STEP4

    
    MOV CX, 180D
    DRAW_MAP_playground_STEP5: 
		mov DX, (170)
		call DRAW_MAP_playground_unit_1	
        add CX, 20
        CMP CX, 280D
    JNZ DRAW_MAP_playground_STEP5  
